...
const SUB_CLAUSE_BOUNDARY_RE = /[,，;；。]/g;
const RATIO_UNITS = new Set(['%', 'pp', 'bps', 'x', '×']);

function isRatioUnit(unit) {
  const raw = String(unit || '');
  return raw === '%' || raw === '×' || RATIO_UNITS.has(raw.toLowerCase());
}
const FOLLOWING_ALIAS_REACH = 12;
const LATIN_WORD_CHAR_RE = /[a-z0-9]/;

function aliasPositions(segment, alias) {
  const positions = [];
  const headIsWord = LATIN_WORD_CHAR_RE.test(alias[0]);
  const tailIsWord = LATIN_WORD_CHAR_RE.test(alias.at(-1));
  let from = 0;
  for (;;) {
    const index = segment.indexOf(alias, from);
    if (index < 0) return positions;
    from = index + 1;
    // "gm" inside "segment" and "eps" inside "steps" are not labels.
    if (headIsWord && index > 0 && LATIN_WORD_CHAR_RE.test(segment[index - 1])) continue;
    if (tailIsWord && LATIN_WORD_CHAR_RE.test(segment[index + alias.length] || '')) continue;
    positions.push(index);
  }
}

function bestAliasHit(segment, score) {
  let best = null;
  METRIC_ALIASES.forEach(([metric, aliases], rank) => {
    aliases.forEach(alias => {
      const lowered = alias.toLowerCase();
      for (const position of aliasPositions(segment, lowered)) {
        const distance = score(position, lowered.length);
        if (distance === null) continue;
        const candidate = [distance, rank, metric, position, lowered.length];
        if (!best || candidate[0] < best[0] || (candidate[0] === best[0] && candidate[1] < best[1])) best = candidate;
      }
    });
  });
  return best ? { metric: best[2], position: best[3], length: best[4] } : null;
}

function bestAlias(segment, score) {
  const hit = bestAliasHit(segment, score);
  return hit ? hit.metric : '';
}

function hasRatioSpanBetween(numberSpans, from, to) {
  return numberSpans.some(([spanStart, spanEnd, ratio]) => ratio && spanStart >= from && spanEnd <= to);
}

function metricFor(text, path, start, metadata = {}, end = start, numberSpans = []) {
  const explicit = canonicalMetric(metadata.metric);
  if (explicit) return explicit;
  const lower = String(text).toLowerCase();
  const boundaries = [...lower.matchAll(SUB_CLAUSE_BOUNDARY_RE)].map(match => match.index);
  // A number belongs to the label that introduces it, so the label is looked for
  // between the number and the nearest thing that could open a new statement:
  // clause punctuation or the previous number. Scoring by distance from the end
  // of a fixed window preferred whatever label FOLLOWED the number, which read
  // "营收 12.3 亿元,归母净利润 2.1 亿元" as a net-profit claim of 12.3 亿元.
  const segmentStart = Math.max(
    0,
    ...boundaries.filter(index => index < start).map(index => index + 1),
    // A growth figure does not open a new statement: in "营收同比增长 20% 至
    // 12.3 亿元" the label 营收 still introduces 12.3 亿元. Only a level number
    // (money, count) closes the segment that introduces the next number.
  ...
  const preceding = lower.slice(segmentStart, start);
  const precedingHit = bestAliasHit(preceding, (position, length) => preceding.length - (position + length));
  const precedingMetric = precedingHit ? precedingHit.metric : '';
  // A label right after the number may claim it ("12.3 亿元的营收") only when
  // nothing introduces it, and never across punctuation or another number.
  const segmentEnd = Math.min(
    lower.length,
    ...boundaries.filter(index => index >= end),
  ...
  // label stays with its ratio and the level goes to its own label.
  ...
  if (claimedAfter) return claimedAfter;
  return bestAlias(pathText(path), () => 0) || 'unclassified';
}
...
function entityClass(metric, baseClass) {
  // A ratio (a 2.0x book-to-bill, a margin, a growth rate) is never an order,
  // contract, or capacity amount, whatever label introduces it.
...
function extractObservations(text, path = [], metadata = {}) {
  ...
  const numberMatches = [...source.matchAll(new RegExp(NUMBER_SOURCE, 'gi'))]
    .filter(match => !ignoredNumericSpans.some(([start, end]) => match.index >= start && match.index < end));
  const numberSpans = numberMatches.map(match => [
    match.index, match.index + match[0].trimEnd().length, isRatioUnit(match.groups?.unit),
  ]);
  for (const match of numberMatches) {
    const metric = metricFor(source, path, match.index, localMetadata, match.index + match[0].trimEnd().length, numberSpans);
...
function normalizeSourceDocuments(sourceDocuments, enrichmentContext, ticker) {
  ...
  // The Dashboard stamps context_built_at at the top level of the context; only
  // reading it under extras left every browser-built source undated, so every
  // money claim was blocked on principle rather than on evidence.
...
export const FACT_CHECK_UNCHECKED_BLOCK_SHAPE = 'UNCHECKED_BLOCK_SHAPE';
const CHECKABLE_BLOCK_TYPES = new Set(['object', 'string']);
...
export function attachFactCheckToBlocks(blocks, enrichmentContext = {}, ticker = '', cutoff) {
  ...
  for (const [name, block] of Object.entries(blocks || {})) {
    // Arrays and strings carry claims too, so they are checked like objects. A
    // block with no shape that can carry a claim (null from a failed agent, a
    // bare number) is reported as unchecked rather than passed in silence.
    ...
    if (!checkable) {
      ...
      receipts[name] = {
        schema_version: FACT_CHECK_SCHEMA,
        status: FACT_CHECK_UNCHECKED_BLOCK_SHAPE,
        block_type: block === null ? 'null' : typeof block,
      };
      continue;
    }
    const receipt = factCheckThesis(block, enrichmentContext, ticker, cutoff);
    receipts[name] = receipt;
    checked[name] = Array.isArray(block) || typeof block === 'string' ? block : { ...block, _fact_check: receipt };
    if (receipt.status === FACT_CHECK_BLOCKED) blocked = true;
  }
  const entries = Object.entries(receipts);
  return {
    blocks: checked,
    receipts,
    status: blocked ? FACT_CHECK_BLOCKED : 'PASS',
    summary: {
      checked_blocks: entries.filter(([, r]) => r.status !== FACT_CHECK_UNCHECKED_BLOCK_SHAPE).length,
      blocked_blocks: entries.filter(([, r]) => r.status === FACT_CHECK_BLOCKED).map(([n]) => n),
      unchecked_blocks: entries.filter(([, r]) => r.status === FACT_CHECK_UNCHECKED_BLOCK_SHAPE).map(([n]) => n),
      fabrication_suspects: entries
