...
def build_decision_block(daily_decision):
    """
    Build a compact decision summary block for Telegram from daily_decision.json.
    Returns a list of lines, or [] if no data.
    """
    if not daily_decision:
        return []

    lines = ["", "<b>▶ 今日交易决策 (Layer 2)</b>"]
    ACTION_ICON = {
        "EXIT": "🚨", "TRIM": "⬇️", "REVIEW_TRIM": "💰",
        "REVIEW_STOP": "🔴", "ADD": "📈", "HOLD": "✅",
        "BUY_WATCH": "🎯", "WATCH": "👁️", "NEUTRAL": "─",
    }

    held = daily_decision.get("decisions", {}).get("held", [])
    for d in held:
        icon = ACTION_ICON.get(d.get("action"), "─")
        ticker = d.get("ticker", "")
        action = d.get("action", "?")
        score  = d.get("confluence", 0)
        pnl    = d.get("pnl_pct")
        pnl_s  = f" P&L {pnl:+.1f}%" if pnl is not None else ""
        lines.append(f"  {icon} <b>{ticker}</b> → {action} "
                     f"(信号{score:+d}{pnl_s})")

    # BUY_WATCH signals from watchlist
    watches = [d for d in daily_decision.get("decisions", {}).get("watchlist", [])
               if d.get("action") == "BUY_WATCH"]
    if watches:
        lines.append("  <i>建仓候选:</i>")
        for d in watches:
            lines.append(f"  🎯 <b>{d.get('ticker')}</b> — {d.get('reason_z', '')[:60]}")

    # Risk flags (condensed)
    risk_flags = daily_decision.get("portfolio_risk", [])
    warn_flags = [f for f in risk_flags if "⚠️" in f or "🔴" in f]
    if warn_flags:
        lines.append("  <i>风险提示:</i>")
...
def build_daily_digest(mdata, flow, daily_decision=None):
    ...
    # Decision block from Layer 2
    decision_lines = build_decision_block(daily_decision)
    if decision_lines:
...
def main():
    ...
    # Load Layer 2 decisions (generated by daily_decision.py earlier in the pipeline)
    daily_decision_path = DATA_DIR / "daily_decision.json"
    daily_decision = None
    if daily_decision_path.exists():
        try:
            with open(daily_decision_path) as f:
                daily_decision = json.load(f)
            print(f"  Loaded daily_decision.json (brief: {daily_decision.get('brief_e', '?')})")
        except Exception as e:
            print(f"  Could not load daily_decision.json: {e}")

    # A. Daily digest (always)
    print("[1/2] Sending daily digest...")
    digest = build_daily_digest(mdata, flow, daily_decision)
