#!/usr/bin/env python3
"""run_v3c_alpha1.py — V3C-α1 backtest runner (v3 Phase B B5).

Per `experiments/agent_tasks/variant_manifests/v3c_alpha1_20260528.json`:
  - 6 windows: full_20yr + wf_2006_2010..wf_2022_2026
  - 3 cost scenarios: optimistic_0.20_RT / baseline_0.40_RT / pessimistic_0.60_RT
  - Total 18 backtests
  - Engine cfg derived FROM manifest design (10d horizon, 4-factor inverse,
    drop rsi_in_band).

The runner:
  1. Reads the manifest (hash verified before run starts).
  2. Builds the engine config from manifest.design + cost_scenario.
  3. Runs each (window, cost_scenario) pair via run_swing_backtest_fast.
  4. Writes one JSON per pair to public/data/v3c_alpha1_<window>_<scenario>.json.
  5. Aggregates baseline-cost rows into public/data/v3c_alpha1_bundle.json
     for v3_gate_eval consumption. Pessimistic + optimistic stored as
     cost_scenarios_runs sibling for IMPL7 multi-cost cross-check.
  6. Invokes v3_gate_eval with stage='R&D'.

Per v3 red line: "宁愿犯错也不愿意找不出来错误在哪". Engine config maps each
manifest field explicitly; any field unmapped is flagged in the run header.

Usage:
    python3 scripts/run_v3c_alpha1.py
        [--manifest <path>]         # default = variant_manifests/v3c_alpha1_20260528.json
        [--out-dir <path>]          # default = public/data/
        [--liquid-top-n N]          # default = 500 (from manifest)
        [--skip-eval]               # write outputs but don't call v3_gate_eval

Exit codes:
    0 = runner OK (gate eval may still report FAIL — that's an honest result)
...
MANIFEST_DEFAULT = REPO_ROOT / "experiments" / "agent_tasks" / "variant_manifests" / "v3c_alpha1_20260528.json"


def _normalize_window_dates(start: str, end: str) -> tuple[str, str]:
    """Normalize YYYY-MM-DD or YYYYMMDD → return (yyyy-mm-dd, yyyy-mm-dd) for engine."""
    def _norm(d: str) -> str:
        s = str(d).strip()
...
def _config_from_manifest(manifest: dict, cost_scenario: str) -> dict:
    """Map manifest.design + cost_scenario → engine cfg dict.

    Explicit field mapping (per task brief; spec mismatches are flagged in
    the runner header for review).
    """
    ...
    # direction: "inverse" → -1, "direct" → +1, "neutral" → 0 (skip).
    ...
    cfg = {
        # Cost dispatch (v3 Phase B B3).
        "cost_scenario": cost_scenario,

        # v3 Phase B-fix (Junyan PM3 2026-05-28 Bug 1): disable the permanent
        # drawdown_to_cash breaker for research backtests. The breaker emits
        # `flat_all` but the spec-comment "wait 5 days re-entry" was never
        # implemented; once triggered in full_20yr the engine stays flat for
        # 14 years (2012-2026). Walk-forward sub-windows didn't expose this
        # because each window starts fresh. v3 research mode disables the
        # action; production paper trading should leave this False until
        # re-entry logic lands.
        "disable_drawdown_breaker": True,

        # Factor wiring (v3 Phase B B1 + per-manifest).
        "factor_directions": factor_directions,
        "signal_weights_override": signal_weights,

        # Inverse-style picking: bottom composite first.
        # The engine's _invert_signal flag picks ascending; factor_directions
        # with -1 also inverts at the per-factor level. We use factor_directions
        # (per-manifest declaration) — not the global _invert_signal — so the
        # composite is computed with each factor in its declared direction and
        # we then pick TOP (high composite = high inverse score).
        "_invert_signal": False,
        "entry_composite_threshold": 50.0,    # composite ≥ 50 entry gate

        # Rebal + hold cadence (Junyan ratify #7).
        ...
        # Universe (per manifest).
        "use_quality_filter": False,
        "_sector_top_k": 99,                  # all sectors
    }
    return cfg
...
def _run_one(window_name: str, start: str, end: str, cost_scenario: str,
             ...
             liquid_top_n: int, manifest: dict, out_dir: Path,
    ...
    audit = res.get("audit", {})
    # Make sure n_dates on the same_gross alpha matches audit.n_trade_days
    # (they're computed from the same series).
    ...
    out_path = out_dir / f"v3c_alpha1_{window_name}_{cost_scenario}.json"
    res_with = {
        **res,
        "v3c_alpha1_window": window_name,
        "v3c_alpha1_cost_scenario": cost_scenario,
        "v3c_alpha1_period": [e_start, e_end],
        "alpha": {"raw": raw_alpha, "same_gross": sg_alpha},
    }
    out_path.write_text(json.dumps(res_with, indent=2, ensure_ascii=False, default=str))

    # Bundle-row shape: matches iter18_b1_b3_walkforward.json per-window dict.
    bundle_row = {
        "variant": "v3c_alpha1",
        "window": window_name,
        "cost_scenario": cost_scenario,
...
def _aggregate_bundle(rows: list[dict], manifest: dict, manifest_path: Path,
    ...
    Bundle conventions:
      - `results` = ONLY the baseline_0.40_RT rows (one per window). This is
        the primary surface that WF1/WF2/IMPL1..7/AUD1..3 read.
      - `cost_scenario_runs` = full grid of 18 rows (window × scenario) so
        IMPL7 can confirm the multi-cost re-run physically happened.
      - `cost_scenario` = "baseline_0.40_RT" (top-level marker for IMPL7
        cross-check: declared scenarios in manifest must match this primary).
      - `benchmarks` = top-level dict copy from the full_20yr/baseline run
        so BENCH gate can verify multi-bench coverage.
    """
    # baseline rows for primary results list
    ...
    # Pick benchmarks dict from baseline full_20yr (else first baseline row).
    bench_source = None
    ...
    # Find associated full result JSON to extract benchmarks (we need the dict
    # itself, not the bundle-row summary).
    bench_dict = {}
    bench_file = bundle_path.parent / f"v3c_alpha1_{bench_source['window']}_{bench_source['cost_scenario']}.json"
    ...
    bundle = {
        "_meta": {
            ...
            "benchmarks_reported": ["EW-500 same-gross", "CSI300", "CSI500", "CSI1000", "cash_2pct"],
        ...
        # Primary results = baseline cost rows. Per task brief, the gate eval
        # MUST run against baseline; pessimistic/optimistic are reported but
        # only enforced at paper/capital promotion stage (IMPL7 §4).
        "results": baseline_rows,
        # Cross-cost runs available for inspection + IMPL7 cross-check.
        "cost_scenario_runs": rows,
        "cost_scenario": "baseline_0.40_RT",   # primary cost scenario
        # Multi-bench dict (Junyan ratify #13).
...
def _verify_manifest_hash(manifest: dict) -> bool:
    """Recompute hash and compare to stored. Returns True if match, False else."""
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        description="V3C-α1 backtest runner (v3 Phase B B5).",
    )
    parser.add_argument("--manifest", type=str, default=str(MANIFEST_DEFAULT),
                        help="V3C-α1 manifest JSON path.")
    ...
    parser.add_argument("--liquid-top-n", type=int, default=None,
                        help="Override manifest liquid_top_n.")
    parser.add_argument("--windows", type=str, default=None,
                        help="Comma-separated window names to run (default: all from manifest).")
    parser.add_argument("--cost-scenarios", type=str, default=None,
                        help="Comma-separated scenarios (default: all from manifest).")
    parser.add_argument("--skip-eval", action="store_true",
                        help="Write outputs but don't invoke v3_gate_eval.")
    ...
    # Hash verification (refuse-run on mismatch).
    if not _verify_manifest_hash(manifest):
        stored = manifest.get("variant", {}).get("hypothesis_lock_hash", "")
        computed = compute_hash(manifest)
        print(f"ERROR: manifest hash mismatch — manifest has been tampered with",
              file=sys.stderr)
    ...
    # Resolve windows + cost scenarios.
    ...
    # Validate scenarios are known to the engine.
    for s in scenarios:
        if s not in COST_SCENARIOS:
            print(f"ERROR: unknown cost_scenario {s!r}; valid: {sorted(COST_SCENARIOS.keys())}",
                  file=sys.stderr)
            return 1
    ...
    print(f"[runner] windows ({len(windows)}): "
    ...
    # Resolve liquid_top_n (manifest default unless CLI override).
    ...
    # Pre-flight: verify momentum_20d wired in DEFAULT_SIGNAL_WEIGHTS.
    ...
    print(f"[runner] panel: {len(panel):,} rows × {panel['ts_code'].nunique()} tickers")
    print()

    # Run grid: window × cost_scenario
    rows = []
    ...
    for r in sorted(full_rows, key=lambda x: x["cost_scenario"]):
        ...
        print(f"{r['cost_scenario']:<22}"
              f"{(r.get('cagr') or 0)*100:+8.2f}%"
              f"{(r.get('max_drawdown') or 0)*100:+8.1f}%"
              f"{(r.get('audit',{}).get('turnover_annual_ratio') or 0)*100:+8.1f}%"
              f"{(sg.get('point',0) if sg else 0):+14.4f}")
    print()
    ...
    cmd = [
        ...
        "--out-md", str(out_dir / "v3c_alpha1_gate_eval.md"),
        "--out-json", str(out_dir / "v3c_alpha1_gate_eval.json"),
    ]
    # IMPL7 gate eval uses stage R&D by default; pass explicitly for transparency.
    # The gate eval CLI does not currently expose --stage, so we rely on the default.
    print("  $ " + " ".join(cmd))
    rc = subprocess.run(cmd).returncode
    print(f"[runner] v3_gate_eval exit={rc} "
          f"({'PASS' if rc == 0 else 'FAIL (gates) — see report'})")

    # Print gate eval markdown for instant readability.
    eval_md = out_dir / "v3c_alpha1_gate_eval.md"
    if eval_md.exists():
        print()
