...
class FactCheckTest(unittest.TestCase):
    ...
    def test_historical_688120_acceptance_sample(self) -> None:
        ...
        self.assertEqual(receipt["status"], "BLOCKED_PENDING_HUMAN")
        # The invented figure reads "Order book ... declines > 10% ... (RMB <8.3B)": the
        # label is separated from the number by another number, so attribution declines
        # to guess a metric. It is still an unsourced monetary figure, and still blocks.
        self.assertTrue(
            any(
                "8.3B" in row["raw"]
                and row["state"] == "UNTRACED"
                and row["entity_class"] in ("MONETARY", "ORDER")
                for row in receipt["fabrication_suspects"]
            )
        )
        self.assertTrue(
            any(
    ...
    def test_top_level_context_built_at_dates_dashboard_shaped_context(self) -> None:
        """src/Dashboard.jsx stamps context_built_at at the TOP of enrichment_context and
        never sends extras. Reading the stamp only under extras left every browser-built
        source undated, so any thesis with a money claim was blocked on principle rather
        than on evidence — the reflexive-BLOCK dual of reflexive-PASS."""
        dashboard_shaped = {
            "context_built_at": "2026-06-10T00:00:00Z",
            "live_price": "48.20",
            "live_change_pct": 1.2,
            "recent_news": [{
                "title": "offline fixture", "source": "fixture",
                "published_at": "2026-06-09T00:00:00Z", "summary": "",
            }],
            "sector_regime": "NEUTRAL",
            "prior_predictions": [],
            "fundamentals": {"pe_trailing": 48, "gross_margin": 41.81, "roe": 12.5, "target_mean": 60.0},
        }
        receipt = self._run_core_request({
            "thesis": {"claim": "Gross margin is 41.81%."},
            "ticker": "688120.SH", "cutoff": CUTOFF, "enrichment_context": dashboard_shaped,
        })
        self.assertGreater(receipt["summary"]["admissible_source_facts"], 0,
                           "a top-level context_built_at must date the enrichment sources")
        ...
        margin = next(
            ...
            for claim in receipt["claims"]
            for observation in claim["observations"]
            if observation["metric"] == "gross_margin"
        )
        self.assertEqual(margin["state"], "TRACED")
        # extras keeps precedence: a future stamp there is not rescued by the top-level one.
        with_future_extras = self._run_core_request({
            "thesis": {"claim": "Gross margin is 41.81%."},
            "ticker": "688120.SH", "cutoff": CUTOFF,
            "enrichment_context": {
                **dashboard_shaped,
                "extras": {"context_built_at": "2027-01-01T00:00:00Z", "filing": "Gross margin is 41.81%."},
            },
        })
        self.assertEqual(with_future_extras["summary"]["admissible_source_facts"], 0)
        self.assertGreater(with_future_extras["summary"]["future_source_facts"], 0)

    def test_multi_metric_clause_attributes_each_number_to_its_own_label(self) -> None:
        """(a) A truthful "营收X,归母净利润Y" against matching evidence: each figure belongs
        to the label that PRECEDES it, and nothing is reported as a contradiction."""
        sentence = "FY2025 营收 12.3 亿元,归母净利润 2.1 亿元。"
        receipt = fact_check_module.fact_check(
            {"claim": sentence}, ticker="688120.SH", cutoff=CUTOFF,
            source_payloads=_source_payload(sentence),
        )
        self.assertEqual(receipt["status"], "PASS")
        self.assertEqual(receipt["summary"]["blocking_mismatches"], 0)
        self.assertEqual(receipt["summary"]["mismatches"], 0)
        ...
        self.assertEqual(by_raw["12.3 亿元"]["state"], "TRACED")
        self.assertEqual(by_raw["2.1 亿元"]["state"], "TRACED")

    def test_right_number_wrong_label_monetary_claim_blocks(self) -> None:
        """(b) "归母净利润 12.3 亿元" where 12.3 is the evidence's 营收 is a monetary
        MISMATCH and blocks. Scoring aliases by distance from the end of a window
        attributed the evidence's 12.3 to the FOLLOWING label 归母净利润, so this
        right-number-wrong-label fabrication traced cleanly."""
        receipt = fact_check_module.fact_check(
            {"claim": "FY2025 归母净利润 12.3 亿元。"}, ticker="688120.SH", cutoff=CUTOFF,
            source_payloads=_source_payload("FY2025 营收 12.3 亿元,归母净利润 2.1 亿元。"),
        )
        self.assertEqual(receipt["status"], "BLOCKED_PENDING_HUMAN")
        self.assertEqual(receipt["summary"]["blocking_mismatches"], 1)
        row = receipt["blocking_mismatches"][0]
        self.assertEqual(row["metric"], "net_profit")
        self.assertEqual(row["raw"], "12.3 亿元")
        self.assertEqual(row["state"], "MISMATCH")
        self.assertEqual(row["entity_class"], "MONETARY")
        self.assertEqual(row["source"]["raw"], "2.1 亿元")

    def test_following_label_claims_a_number_only_when_nothing_precedes_it(self) -> None:
        receipt = fact_check_module.fact_check(
            {"claim": "FY2025 实现 12.3 亿元营收,归母净利润 2.1 亿元。"}, ticker="688120.SH",
            cutoff=CUTOFF,
            source_payloads=_source_payload("FY2025 营收 12.3 亿元,归母净利润 2.1 亿元。"),
        )
        by_raw = {
        ...
        self.assertEqual(by_raw["12.3 亿元"]["metric"], "revenue")
        self.assertEqual(by_raw["12.3 亿元"]["state"], "TRACED")
        # A label after the punctuation cannot reach back across it.
        unlabelled = fact_check_module.fact_check(
            {"claim": "FY2025 实现 12.3 亿元,归母净利润 2.1 亿元。"}, ticker="688120.SH",
            cutoff=CUTOFF,
            source_payloads=_source_payload("FY2025 营收 12.3 亿元,归母净利润 2.1 亿元。"),
        )
        first = next(
            observation
            for claim in unlabelled["claims"]
            for observation in claim["observations"]
            if observation["raw"] == "12.3 亿元"
        )
        self.assertEqual(first["metric"], "unclassified")

    def test_growth_then_level_phrasing_keeps_the_preceding_label(self) -> None:
        """"营收同比增长 20% 至 12.3 亿元": the growth figure must not strip 营收 from
        the level figure that follows it (r2 finding), in Chinese and in English IR
        phrasing; the wrong-label form still blocks."""
        for sentence, raw in (
            ("FY2025 营收同比增长 20% 至 12.3 亿元。", "12.3 亿元"),
            ("FY2025 revenue rose 15% YoY to RMB 4.6B.", "RMB 4.6B"),
        ):
            with self.subTest(sentence=sentence):
                ...
                self.assertEqual(by_raw[raw]["metric"], "revenue")
                self.assertEqual(by_raw[raw]["state"], "TRACED")
        wrong = fact_check_module.fact_check(
            {"claim": "FY2025 归母净利润同比增长 20% 至 12.3 亿元。"}, ticker="688120.SH",
            cutoff=CUTOFF,
            source_payloads=_source_payload("FY2025 营收 12.3 亿元,归母净利润 2.1 亿元。"),
        )
        self.assertEqual(wrong["status"], "BLOCKED_PENDING_HUMAN")
        self.assertEqual(wrong["summary"]["blocking_mismatches"], 1)
        self.assertEqual(wrong["blocking_mismatches"][0]["metric"], "net_profit")

    def test_level_after_a_ratio_goes_to_its_own_following_label(self) -> None:
        """r3 regression: "gross margin 41.8% on RMB 4.6B revenue" — once ratio spans stop
        bounding the segment, the nearest preceding label is the RATIO's label; the level
        figure must go to the label that follows it, in English and in Chinese."""
        for sentence, raw, metric in (
            ("FY2025 gross margin 41.8% on RMB 4.6B revenue.", "RMB 4.6B", "revenue"),
            ("FY2025 毛利率提升至 41.8% 带动 2.1 亿元归母净利润。", "2.1 亿元", "net_profit"),
        ):
            with self.subTest(sentence=sentence):
                receipt = fact_check_module.fact_check(
                    {"claim": sentence}, ticker="688120.SH", cutoff=CUTOFF,
                ...
                self.assertEqual(by_raw[raw]["metric"], metric)
                self.assertEqual(by_raw[raw]["state"], "TRACED")
                self.assertEqual(receipt["status"], "PASS")
        # Without a label after the level, the growth label still introduces it.
        receipt = fact_check_module.fact_check(
            {"claim": "FY2025 营收同比增长 20% 至 12.3 亿元,归母净利润 2.1 亿元。"}, ticker="688120.SH",
            cutoff=CUTOFF,
            source_payloads=_source_payload("FY2025 营收 12.3 亿元,归母净利润 2.1 亿元。"),
        )
        by_raw = {o["raw"]: o for c in receipt["claims"] for o in c["observations"]}
        self.assertEqual(by_raw["12.3 亿元"]["metric"], "revenue")
        self.assertEqual(by_raw["2.1 亿元"]["metric"], "net_profit")
        self.assertEqual(receipt["status"], "PASS")
    ...
    def test_ratio_multiple_is_never_upgraded_to_an_order_class(self) -> None:
        ...
        amount, so it never becomes a blocking-class suspect on its own."""
        sentence = "Order book 2.0x book-to-bill in FY2025."
        receipt = fact_check_module.fact_check(
            {"claim": sentence}, ticker="688120.SH", cutoff=CUTOFF,
            source_payloads=_source_payload(sentence),
        )
        row = next(
            ...
            for claim in receipt["claims"]
            for observation in claim["observations"]
            if observation["raw"].startswith("2.0")
        )
        self.assertEqual(row["metric"], "order_book")
        self.assertEqual(row["entity_class"], "RATIO")
        self.assertEqual(receipt["summary"]["blocking_mismatches"], 0)

    def test_array_shaped_block_is_checked_and_unsupported_shape_is_explicit(self) -> None:
        """An array-shaped thesis block carries claims like any object and must be
        checked; a block with no claim-bearing shape is reported as unchecked, never
        passed in silence."""
        runner = """
import { pathToFileURL } from 'node:url';
const [corePath] = process.argv.slice(2);
const { attachFactCheckToBlocks } = await import(pathToFileURL(corePath).href);
const out = attachFactCheckToBlocks(
  {
    _technical: [{ claim: 'FY2025 order book was RMB 8.3B.' }],
    _bear_r1: 'FY2025 order book was RMB 8.3B.',
    _forensic: null,
    _bull_r1: 7,
  },
  { extras: { context_built_at: '2026-06-10T00:00:00Z', filing: 'FY2025 gross margin was 41.81%.' } },
  '688120.SH', '2026-06-11T23:59:59Z');
process.stdout.write(JSON.stringify({ ...out, technical_is_array: Array.isArray(out.blocks._technical) }));
"""
        output = self._run_node(runner, CORE_PATH)
        receipts = output["receipts"]
        self.assertEqual(receipts.get("_technical", {}).get("status"), "BLOCKED_PENDING_HUMAN",
                         "an array-shaped block left the gate unchecked")
        self.assertTrue(output["technical_is_array"], "checking must not reshape the array")
        self.assertEqual(receipts.get("_bear_r1", {}).get("status"), "BLOCKED_PENDING_HUMAN")
        self.assertEqual(receipts.get("_forensic", {}).get("status"), "UNCHECKED_BLOCK_SHAPE")
        self.assertEqual(receipts.get("_bull_r1", {}).get("status"), "UNCHECKED_BLOCK_SHAPE")
        self.assertEqual(output["status"], "BLOCKED_PENDING_HUMAN")
        self.assertEqual(sorted(output["summary"]["blocked_blocks"]), ["_bear_r1", "_technical"])
        self.assertEqual(sorted(output["summary"]["unchecked_blocks"]), ["_bull_r1", "_forensic"])
        self.assertEqual(output["summary"]["checked_blocks"], 2)

    def test_multi_api_quality_cap_applies_to_fabricated_sub_theses(self) -> None:
        """The multi-agent _quality must carry the fabrication cap when any returned
        block holds suspects, not only the synthesis (F-029 on the multi path)."""
        output = self._run_node(NODE_HANDLER_RUNNER, RESEARCH_MULTI_API, "multi")
        quality = output["body"]["data"].get("_quality") or {}
        self.assertTrue(quality.get("fabrication_capped"),
                        "multi-agent _quality escaped the fabrication cap")
        self.assertGreaterEqual(quality.get("fabrication_suspects", 0), 1)
        self.assertLessEqual(quality.get("score", 101), 40)
        self.assertNotEqual(quality.get("severity"), "PASS")

    def test_multi_api_sub_block_alone_blocks_the_envelope(self) -> None:
        """Synthesis clean, sub-theses fabricated: the envelope _status must still read
        BLOCKED, because the held blocks are returned and quotable regardless."""
        clean_synth_loader = SDK_LOADER.replace(
            "FY2025 order book was RMB 8.3B.", "FY2025 gross margin was 41.81%."
        )
        self.assertNotEqual(clean_synth_loader, SDK_LOADER)
        output = self._run_node(
            NODE_HANDLER_RUNNER, RESEARCH_MULTI_API, "multi", loader_source=clean_synth_loader
        )
        body = output["body"]
        self.assertEqual((body["data"].get("_fact_check") or {}).get("status"), "PASS",
                         "fixture drift: the synthesis was meant to be clean")
        self.assertEqual(body.get("_pipeline_status"), "OK")
        sub_theses = body["data"].get("_fact_check_sub_theses") or {}
        self.assertIn("_bull_thesis", sub_theses.get("blocked_blocks", []))
        self.assertEqual(body["_status"], "BLOCKED_PENDING_HUMAN")
