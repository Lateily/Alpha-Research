...
class GovernanceMutationGateTests(unittest.TestCase):
    ...
    def test_validate_manifest_enforces_a035_marker_coverage(self) -> None:
        case = gate.MutationCase(
            mutation_id="AIOS_A035_SYNTHETIC_GATE",
            component="AIOS A-035 synthetic",
            source_path="source.py",
            test_script="test_source.py",
            before="guard = True",
            after="guard = False",
            expected_failure_marker="synthetic",
            rationale="Synthetic case proving A-035 marker coverage is load-bearing.",
        )
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "source.py").write_text("guard = True\n", encoding="utf-8")
            ...
            for relative in gate.A035_GOVERNANCE_PATHS:
                marker_path = root / relative
                marker_path.parent.mkdir(parents=True, exist_ok=True)
                marker_path.write_text("# missing marker\n", encoding="utf-8")
            with self.assertRaisesRegex(
                gate.MutationGateError,
                "mutations_without_markers.*AIOS_A035_SYNTHETIC_GATE",
            ):
                gate.validate_manifest(root, [case])

    def test_validate_manifest_enforces_a020_marker_coverage(self) -> None:
        case = gate.MutationCase(
            mutation_id="AIOS_A020_SYNTHETIC_GATE",
            component="AIOS A-020 synthetic",
            source_path="source.py",
            test_script="test_source.py",
            before="guard = True",
            after="guard = False",
            expected_failure_marker="synthetic",
            rationale="Synthetic case proving A-020 marker coverage is load-bearing.",
        )
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "source.py").write_text("guard = True\n", encoding="utf-8")
            ...
            for relative in (
                ...
                *gate.FUNNEL_NIGHTLY_GOVERNANCE_PATHS,
                ...
                marker_path.parent.mkdir(parents=True, exist_ok=True)
                marker_path.write_text("# no relevant markers\n", encoding="utf-8")
            for relative in gate.A020_GOVERNANCE_PATHS:
                marker_path = root / relative
                marker_path.parent.mkdir(parents=True, exist_ok=True)
                marker_path.write_text("# missing marker\n", encoding="utf-8")
            with self.assertRaisesRegex(
                gate.MutationGateError,
                "mutations_without_markers.*AIOS_A020_SYNTHETIC_GATE",
            ):
                gate.validate_manifest(root, [case])
