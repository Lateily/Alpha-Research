...
3. Add these EXTRA fields to your output:
   ...
   "_validation_direction": "LONG | SHORT | WATCH_LONG | WATCH_SHORT | PASS | UNRESOLVED — the Alpha-Factory FORWARD-VALIDATION label. This is a research / tracking label ONLY: never a capital order, never auto-positioned (no_trade_flag). Derive it AFTER _direction, and keep it CONSISTENT with scripts/backfill_thesis_direction.py so backfilled and native records share one taxonomy. Rules: (1) _direction is LONG/SHORT -> emit the SAME LONG/SHORT. (2) _direction is PASS BUT a ONE-SIDED business mechanism exists AND all of {named catalyst, horizon, proves_right_if, proves_wrong_if, confidence, numeric reward:risk >= 1.5:1, evidence mostly [E1]/[E2]} hold AND it misses capital LONG/SHORT only for a single named requirement -> emit WATCH_LONG or WATCH_SHORT on the side the asymmetry/mechanism leans. (3) direction symmetric/both-ways OR ambiguous OR any required field missing -> UNRESOLVED. (4) genuinely no edge / no data / balanced R:R / explicit no-position / capital-preservation -> PASS.",
   "_validation_direction_note": "REQUIRED literal tag '[unvalidated forward candidate]' when _validation_direction starts with WATCH_ ; otherwise null",
   "_validation_direction_basis": "1 sentence naming which rule (1-4) fired and the one-sided signal (asymmetry sign / numeric R:R / named catalyst) that justified WATCH vs PASS vs UNRESOLVED"
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: thesisOut,
      models_used: MODELS,
      // A clean synthesis over a blocked sub-thesis is still a blocked response:
      // the held block is returned, rendered and quotable either way.
