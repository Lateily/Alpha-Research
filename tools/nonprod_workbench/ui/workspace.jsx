  ...
  return <div className="workspace-surface">
    ...
    {!state ? <Empty title="工作台状态读取中" /> : <>
      ...
      {tab === 'desk' && <>
        ...
        <section><div className="section-title"><h2>已发布 run 的研究质量</h2><Status value={quality.status || 'NOT_EVALUATED'} /></div><div className="status-lines"><div><span>来源覆盖</span><Status value={quality.macro ? (quality.macro.source_coverage_complete ? 'MATCH' : 'MISMATCH') : 'UNVERIFIED'} /></div><div><span>事件覆盖</span><Status value={quality.macro ? (quality.macro.event_coverage_complete ? 'MATCH' : 'MISMATCH') : 'UNVERIFIED'} /></div><div><span>来源缺行</span><strong>{quality.macro ? `${quality.macro.missing_source_rows} / ${quality.macro.expected_sources}` : '未验'}</strong></div><div><span>事件缺行</span><strong>{quality.macro ? `${quality.macro.missing_event_rows} / ${quality.macro.expected_events}` : '未验'}</strong></div><div><span>宏观不可用</span><strong>{quality.macro ? `${quality.macro.unavailable_sources} / ${quality.macro.sources_total}` : '未验'}</strong></div><div><span>实际值缺失</span><strong>{quality.macro ? `${quality.macro.actual_blocked_events} / ${quality.macro.events_total}` : '未验'}</strong></div><div><span>缺预期差</span><strong>{quality.macro ? `${quality.macro.missing_consensus} / ${quality.macro.events_total}` : '未验'}</strong></div><div><span>候选零维</span><strong>{quality.funnel ? `${quality.funnel.coverage.zero} / ${quality.funnel.candidate_count}` : '未验'}</strong></div><div><span>未开始电池</span><strong>{quality.funnel?.batch_not_started ?? '未验'}</strong></div></div><div className="source-strip"><span>观察 run <code>{quality.run_id || 'UNAVAILABLE'}</code></span><span>只读计数，不等于自动轮验收或研究批准</span></div></section>
      ...
      {tab === 'candidates' && <>
        <section><div className="section-title"><h2>本轮候选电池覆盖</h2><Status value={quality.status || 'NOT_EVALUATED'} /></div><div className="status-lines"><div><span>六维齐全</span><strong>{quality.funnel?.coverage.complete ?? '未验'}</strong></div><div><span>部分缺维</span><strong>{quality.funnel?.coverage.partial ?? '未验'}</strong></div><div><span>六维皆缺</span><strong>{quality.funnel?.coverage.zero ?? '未验'}</strong></div><div><span>未开始采集</span><strong>{quality.funnel?.batch_not_started ?? '未验'}</strong></div></div><div className="source-strip"><span>发布 run <code>{quality.run_id || 'UNAVAILABLE'}</code></span><span>不替代 U4 准入校验</span></div></section>
