...
大白话:每笔交易一张病历卡,按 month 字段(YYYYMM 月)分组展示,不堆总表。

每张卡:ticker/name/status(filled|closed|cancelled)/fill_date/entry/qty/
stop(灾难线)/target/exit_date/exit_price/realized_r/
reasoning_trail(最近≤6条决策日志,含两票与复核记录)/month(分组键;真实账本字段映射:realized_R→realized_r、stop_reference→stop、take_profit_reference→target)。
