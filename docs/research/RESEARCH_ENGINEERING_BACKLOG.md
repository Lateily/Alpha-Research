...
| R-008 | 本地特征仓与增量计算 | `DELIVERED_UNWIRED` | Codex+Data Agent | **本分支待 PR**:SQLite v0 已保存 `daily/daily_basic/adj_factor` 三类批量事实,按交易日三源原子提交并生成复权 1/5/10/20 日收益、20 日收盘高点距离、成交额、换手、估值与市值特征。20260805 首跑回填 25 日,最新日 5,532 行特征,5 只无当日行情逐只列为缺口;本地库约 81 MB 且受 `.gitignore` 保护,repo 仅提交健康契约。**未上线边界**:尚未接夜链/消费者,资金流尚未进入 v0 | `feature_store.py --selftest` 13/13;全套 `AR_OFFLINE=1` 离线测试通过;真实二次运行 `ingested=0/idempotent_skips=25`,批次哈希与特征哈希均不变;同日源修订、三源缺失、行情缺复权、历史逆序插入、覆盖错账全部 fail-closed |
...
| R-032 | U0 全市场证券注册表 | `DELIVERED_UNWIRED` | Codex+Data Agent | **PR #226**:生成器+schema+首份 20260805 实物已交付;真实注册 5,876 只,当前上市/U1 可扫描 5,537 只,退市保留 339 只。3 只新股流动性历史不足,顶层诚实为 PARTIAL。R-036 已消费该注册表并完成全市场 join;**剩余未接线边界**:注册表自身尚未进入夜链日更 | 全部 A 股永久注册+资格标签;原子写;重复/损坏 prior/哈希与覆盖漂移 fail-closed;selftest 17/17;Tushare `stock_basic/trade_cal/daily` 首跑完成 |
...
| R-036 | 全市场 E1 事件层 | `DELIVERED_UNWIRED` | Codex+Data Agent | **PR #228 已合并**:消费 R-032 的 5,537 只 U1 可扫描证券,以 12 次 Tushare VIP 批量调用替代约 1.6 万次逐票请求;首跑 1,319 RED_FLAG / 4,203 NO_RED_FLAG_FOUND / 15 DATA_BLOCKED。修复旧闸门把 `yoy_net_profit` 金额误读为百分比的语义 bug;正式财报终止同报告期及更早的预告/快报;触发证据按 E1 留痕。**诚实边界**:`anns_d` 正式公告权限仍 DATA_BLOCKED,顶层因此为 PARTIAL;尚未接夜链日更 | generator + schema + 20260805 全市场契约;selftest 22/22;全 5,537 行一一对应 U0 且零重复;12 个批量源调用零错误;断网时 5,537 行全部 DATA_BLOCKED、不沿用旧结果;接线须待 nightly-v4 文件 CLAIM 释放并单独验收 |
