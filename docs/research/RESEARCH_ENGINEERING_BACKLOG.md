...
|---|---|---|---|---|---|
| R-001 | 夜链终态必须服从子契约语义 | `DONE` | Claude | **#236 + #256 + #269 已把执行完整度与数据质量拆成正交维度**:业务步骤失败仍令 `report=INCOMPLETE/published=false`;结构合法的 `PARTIAL/DATA_BLOCKED` 进入顶层 `data_quality/degraded_sources` 而不冒充完整数据,Macro/漏斗校准步骤失败保持隔离但必须报警。2026-08-15 手动生产 canary 实证 `report=COMPLETE/published=true/data_quality=DATA_BLOCKED`,22 步全 OK,降级来源完整可见 | **验收通过 2026-08-15**:业务失败阻断发布、诚实降级可发布、隔离失败仍报警三条均有端到端负向回归;不得退回“全绿才发布”或“降级静默消失” |
...
| R-005 | Macro OS 与 MRG 完整闭环 | `VALIDATING` | Junyan+Macro Agent | **M0-A #240、M0-B #245、M0-B2 #247、M0-B3 #249、M1-A #251、M1-B #252、M1-C #256 均已合并并同步运行目录**。2026-08-15 手动生产 canary 已产出同轮 Macro 合约与 manifest,终态 `CALIBRATING / DATA_BLOCKED`,机器维持 `enforceable=false/formal_blocking_authority=false`,缺证据时风险预算 `UNCHANGED/CALIBRATION_NO_MACRO_EVIDENCE`。尚缺官方发布日历物化器,因此 `RELEASE_CALENDAR_NOT_PUBLISHED` 继续 fail-closed | 2026-08-17 首次 launchd 自动轮验收;新增官方日历抓取/快照/投影物化器并完成一次真实 due/reuse 轮;随后接 T+1/T+5/T+20 判分影子运行。校准期不得声称正式 regime 或阻断权 |
...
| R-022 | 夜链报警消费闭环 | `IN_PROGRESS` | Ops Agent | #269 已让隔离失败也落 `/tmp/ar-nightly-incomplete`,但“旗存在”仍不等于有人消费。本批新增只读 `nightly_acceptance.py`:绑定安装 plist/wrapper/runner、launchctl 计数与退出码、精确 run 日志段、持久 bundle 和报警旗,拒绝拿旧日志 OK 行冒充本轮自动验收 | 2026-08-17 16:35 用上线前 `runs_before` 生成 PASS receipt;后续仍需 owner/ack/处理结果、未确认报警进入次日 preflight和最近三晚健康查询,完成前保持 IN_PROGRESS |
...
| R-031 | MRG 两阶段接线 | `VALIDATING` | Claude+Macro Agent | **#251/#252/#256 已把校准边界接入生产夜链并通过 2026-08-15 手动 canary**。只有 `CREDIT_STRESS_CANDIDATE` 有完整证据时才把 `REDUCED_REVIEW_BUDGET` 映射为 `STARTER_CAPPED`;`MACRO_PARTIAL` 保持 `UNCHANGED/CALIBRATION_NO_MACRO_EVIDENCE`。机器继续强制 `enforceable=false`、`formal_blocking_authority=false`,并禁止交易动作/直接阻断/正式 regime | 阶段A在 launchd 自动验收后进入每日影子判分;阶段B 达 ≥30 独立因果簇且优于随机后,再由 Junyan 单独批准正式阻断权 |
...
| R-032 | U0 全市场证券注册表 | `VALIDATING` | Codex+Data Agent | **#226 + #234 已合并部署,#269 已将其接入完整漏斗**。2026-08-15 手动生产 canary 与 U1/U2 同轮共享 `AR_TARGET_TRADE_DATE`,eligible universe 5,543 只;重复/损坏 prior/哈希、日期错配与覆盖漂移继续 fail-closed。**验证边界**:首次 launchd 16:35 正式值班尚待验收 | 2026-08-17 自动轮 receipt PASS;全部 A 股永久注册+资格标签、原子写和运行时 preflight 持续通过 |
| R-033 | U1 六通道批量扫描器 | `VALIDATING` | Claude+Scanner Agent | **#267 契约、#269 隔离夜链接线已合并并完成手动生产 canary**:5,543 只 eligible 证券形成 33,258 条通道观察;六通道独立、禁复合总分、entry_reasons 留痕均由契约和 mutation gate 强制。真实终态 `PARTIAL`,各通道缺失没有被平均或洗白 | 2026-08-17 自动轮 receipt PASS;补齐仍 DATA_BLOCKED 的宏观/资金/基本面通道,在校准前保持相对阈值 `UNVALIDATED` |
| R-034 | U2 候选池与三项保留配额 | `VALIDATING` | Claude+Scanner Agent | **#267/#269 已合并并完成手动生产 canary**:105 个 `U3_BATTERY_REVIEW` 候选;逆向修复 15/15、随机控制 10/10、慢牛 0/15,目标 200 短缺 95。淘汰理由、控制批次与抽样框已留痕,U4 仍结构性要求 Junyan 显式选票 | 自动轮稳定后修慢牛配额与候选短缺;由 Junyan 从 ready pool 每周选 3–5 家进入 U4,机器不得代选 |
| R-035 | 随机控制分组判分 | `DELIVERED_UNWIRED` | Validation Agent | **离线评估器已交付**:只读不可变漏斗 bundle、同日 U3 电池与 R-008 SQLite,以 U2 登记日同一 settled close 为 t0,分别计算 T+1/3/5/10 后复权收益;停牌按目标日前最后可得定盘价截断并显式标 `TRUNCATED`。U1/U2 主通道 vs 同批随机控制与 U3 电池 PASS vs NON_PASS 是两项独立检验,禁止跨批混算、禁止把收益写回候选池。保留配额样本可进入 U3,但明确不并入 U1/U2;慢牛/逆向信息量需另行预注册 cohort test | 14 道承重门进入 mutation-gate;样本数不得解锁 claim,`claim_allowed=false` 且历史贡献为 0,继续阻断于 R-038/039/040/041。**尚未接夜链或发布面**,接线须另开 PR 并先完成自动 launchd 生产验收 |
| R-036 | 全市场 E1 事件层 | `VALIDATING` | Codex+Data Agent | **#228 + #234 已合并部署,#269 已把其证据接入 U1/U2**。手动生产 canary 中 15 个 E1 通道 DATA_BLOCKED,红旗对象只进入 `E1_REVIEW_ONLY`,不得作为正向候选。正式财报继续终止同报告期及更早预告/快报。**诚实边界**:`anns_d` 权限仍 DATA_BLOCKED,顶层因此为 PARTIAL | 2026-08-17 自动轮 receipt PASS;断网全行 DATA_BLOCKED、不沿用旧结果;后续取得正式公告权限或经验证 E1 替代源 |
...
| R-043 | publication manifest migration | `DONE` | Codex+Junyan | **#259 + #258 已合并并在 0806 生产现场实战验证**:按批准原文和冻结 `plan_hash` 在夜链锁内迁移 #241/#242 引起的两项哈希分歧;独立复验 `problems=[]`,WAL 恰好一条 intent + 一条 commit、无 abort,ET/public manifest 与双 pointer 分别逐字节一致,preflight PASS。随后 #264 修复跨时区账本时间生成并回归验证 | **验收通过 2026-08-12**:工具和生产现场均证明可恢复 WAL、TOCTOU、双 manifest/pointer、NOOP 全量复验与唯一终态;后续迁移仍须新 plan_hash + Junyan 明确授权,不得把 DONE 理解为常规写权限 |
...
| Macro M1-A/M1-B/M1-C | `VALIDATING / CALIBRATING` | 同轮编排与夜链 staged publish 已在代码层接通;不等于已部署、已有正式产物/regime 或拥有阻断权 |
