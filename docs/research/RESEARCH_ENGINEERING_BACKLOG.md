...
| R-031 | MRG 两阶段接线 | `VALIDATING` | Claude+Macro Agent | **PR #251/#252 已把校准边界写进机器契约**。M1-C 本批将 `REDUCED_REVIEW_BUDGET` 映射为 `STARTER_CAPPED` 风险预算标注,同时机器强制 `enforceable=false`、`formal_blocking_authority=false`,并禁止交易动作/直接阻断/正式 regime;尚未部署或进入每日判分 | 阶段A完成真实夜跑后每日入判分;阶段B 达 ≥30 独立因果簇且优于随机后,再由 Junyan 单独批准正式阻断权 |
