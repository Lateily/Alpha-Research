...
**① 同池**:随机样本必须从**与主通道完全相同的合格证券池**抽取 ——
即通过 U0 资格标签检查(流动性下限、非退市整理、非停牌)后的同一集合。
...
**③ 固定 seed**:`seed = hash(trade_date + "AR_RANDOM_CONTROL_v1")`,写入产物。
同一交易日重跑必须得到**完全相同**的随机样本 —— 否则可以反复重抽直到随机组表现难看,
这是最隐蔽的一种作弊。

**④ 记录抽样框(sampling frame)**:每期产物必须完整保存
`{as_of, eligible_universe_n, eligible_universe_hash, strata[], quota_per_stratum,
seed, drawn[], excluded_with_reason[]}`。
...
| **U3 层检验**(研究增值) | 通过六维电池的票 vs 未过电池的候选 | **研究环节**有没有增值:跑完六维再筛一道,是否比只靠 U2 更好 | 两者无法区分 ⇒ 六维电池未产生增量,须重构维度或阈值 |

**两层必须分开的原因**:若只做一层总检验,"发现层有用但研究层无用"与
"发现层无用但研究层救回来了"这两种完全相反的情况会得出同样的总分,
...
- U3 层无增值 ⇒ 六维电池的维度或阈值须重构,不得因"跑了很多数据"而默认有用;
