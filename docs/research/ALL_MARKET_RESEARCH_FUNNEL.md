...
随机控制配额**不是**为了选股,而是为了回答一个我们此前无法回答的问题:
**这个漏斗到底有没有信息量?**

**检验方式**:随机控制样本与主通道候选**走完全相同的下游流程**
(同样进 U3 六维电池、同样回填 T+1/3/5/10 远期收益、同样入判分池),
但在判分时**单独成组**。

**判据(预注册,不可事后修改)**:
- 若主通道候选的前瞻收益分布与随机控制**统计上无法区分**
  ⇒ **漏斗在该期无信息量**,必须在周报中如实声明,并进入通道重构;
- 若某单一通道长期不优于随机控制 ⇒ 该通道**降级或下线**;
- **样本门槛照 v1.5 C5 执行**:按独立事件簇计数,constructive/cautious 分列,
  未达标前一律不得声称漏斗"有效"。

**这条设计的意义**:它把"我们的研究流程有没有用"从一个信念问题,
变成一个每期都在被测量的问题。**没有随机控制的漏斗,永远只能自证。**
...
| R-035 | 随机控制分组判分(负控制口径,预注册判据) | R-034 + v1.5 C5 |
| R-036 | 全市场 E1 事件层(红旗闸门从 14 只扩到全 A,批量口径) | R-032(原 R-007 并入)|
