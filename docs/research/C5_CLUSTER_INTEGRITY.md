...
> 给不出该证明的修改无效。**
>
> **有意收紧的豁免**:若该修改的**目的本身**就是把某类现行写手行为排除出 S0
> (例如判定盘中 `14:37 (intraday, 定盘判分)` 的登记不应算 S0),
> 则可用**Junyan 批准(§0.4)+ 配套的写手改动**替代上述证明 ——
> 写手改动须使修改后的默认字段重新满足活性,并与本次修改同批落地。
> 无此豁免,本条就是钉在执行方可控代码上的无条件棘轮:任何以“排除现行行为”
> 为目的的正当收紧都自动无效,连按其是非讨论的机会都没有。
> **守门条款过紧与过松同样是失效** —— 这是本文档四轮里反复付过学费的一课。
...
#### 0.3.1.1 本版修改的活性证明(即 §0.3.1 要求的交付物)

**测量基准**:2026-08-03 工作树账本快照,**128 行(已计分 93 行)**。
分支 blob 是 122 行的较早快照,同样的计算在其上得 S0=0 —— 两者都对,
差异只是快照日期;引用本表数字时须连同基准一起引用。

对生产写手 `execution_tracker.py --log` 的默认字段
(`timestamp: "<D> close (official)"` · `horizon: ["intraday","1d","3d","5d","10d"]` ·
`returns: {}`),在 D 日盘后登记:

| 检验 | 计算 | 结果 |
|---|---|---|
| **活性** D 日当天 | `registered_trade_date = D`;`今日(D) > D = False`;`returns_empty = True` | **S0** ⇒ §0.3 例外覆盖 ⇒ `retrospective: false` ⇒ **计入 §0.1** ✓ |
| **安全性** D+1 交易日 | `D+1 > D = True` | **S1**,S0 正确终止(1d bar 已结算) ✓ |
| **三轮 60d 洞** 0501 登记 `["20d","60d"]` 并压住回填 | `20260803 > 20260501 = True` | **S1**,洞保持关闭;S0 终止与 `max_horizon` 无关 ✓ |
| **现存账本** 按本版判据 | S0 = 6 行,登记日分布 `{20260803}` | 全部恰为当日登记(新易盛/香农芯创/利通/旭创/恒瑞/牧原),语义正确 ✓ |

对照:四版判据下这套默认字段产出 **S1**(因 intraday 析取项),
`independent_clusters` 恒为 0;三版判据下账本里有 **13 行**登记于 0711–0722 的行
被误判为 S0。本版两者皆已消除。
...
(五轮 must-fix:四版一边声明“全文唯一定义”,一边把六条规范条款在此处又抄了一遍,
其中就包括 §0.4 的立论所指认的那句“一次批准只能被一条 migration 引用”。
两份副本冻结在同一份文档里,正是本文件修订史指认的条款漂移病 —— 副本已删。)
...
- `claim_frozen_until` = **该路径的起算日** + 一个完整判分周期,**必须写成具体日期**:
  P2 取 manifest 落盘日,P1/P3 取上条定义的首次达标信号登记日。
  **P1/P3 无 manifest,故该日期由 `scorecard()` 计算并写入当期 publication snapshot
  的 `claim_frozen_until` 字段**(R-041 交付),不得因“没有 manifest 可写”而落空。
  (五轮 BLOCKER:四版只改了冻结**起点**去绑定 P1,终点仍写“落盘日 + …”,
  而落盘日只有 P2 有 ⇒ P1 路径起点有定义、终点无定义,可被读成“冻结期为空”,
  正好反转 MF-3 想修的东西;而 P1 是目前唯一活路径。)
