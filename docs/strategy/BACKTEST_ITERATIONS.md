...
| **10** | **Quarterly rebalance** — Stage 2 IC at h=3m / 6m was 2-3× stronger than h=1m. Hypothesis: rebalancing quarterly to match the strongest-signal horizon would improve OOS alpha. | `--rebal-months 3` flag in `run_universe_backtest.py` (also supports 6 for semi-annual). Quarterly = 82 rebalances over 20yr vs monthly 245. | **Quarterly**: OOS 2.87% / IS 1.77% / MaxDD -36.9% / Sharpe 0.37 / final 1.44. **Semi-annual**: OOS 3.97% / IS -3.25% / MaxDD -54.6% / Sharpe 0.14 / **final 0.87 (LOSS)**. Bootstrap CI on quarterly ALPHA annualized = **-31.8% CI [-56.2%, -1.9%] CI EXCLUDES 0 (p=0.022) — statistically significant NEGATIVE alpha.** | **REJECTED** | **不能负优化 violated** — quarterly produces statistically significant NEGATIVE alpha vs monthly Stage 1b's straddling-zero CI. **Root cause**: Stage 2 IC at h=3m measures forward signal from EVERY MONTH-END, not from quarter-ends. The signal IS stronger at 3m holding period — but rebalancing only 4×/year MISSES 8 months of entry opportunities. **Lesson**: signal horizon ≠ rebalance frequency. Correct way to harvest the 3m signal: monthly rebal + 3-month factor smoothing (defer to iter-11 idea), NOT quarterly rebal. Semi-annual is even worse (final equity < 1.0 = 20-year LOSS). |
| **11** | **Monthly rebal + 3-month factor smoothing** — the "correct" way to harvest Stage 2 IC h=3m signal per iter-10 lesson. Average factor scores over last 3 month-ends, then composite + rank. | `--smooth-n 3` flag. Per rebalance: 3× compute_factors → ~3× slower but PIT-clean (each lookback queries store at its own as_of). | OOS **8.26%** (vs Stage 1b 10.50%, **-2.24pp**) / IS 3.33% / MaxDD -47.7% / Sharpe 0.41 / final 2.65 / 2015 bubble +143.95% (vs 129%) / 2020 covid +8.92% (vs 14%). Bootstrap CI: ALPHA annualized point -9.79% CI [-21.5%, +1.5%] p=0.098 — straddles 0. | **NEUTRAL / REJECTED** | **Smoothing didn't help**: OOS lower (-2.24pp) but bootstrap CI overlaps with Stage 1b's CI [-21.3%, +2.2%] — statistically same bucket of "no demonstrable edge". The 3-month smoothing dilutes the entry-month factor measurement instead of "capturing" the h=3m signal. The HONEST reading of Stage 2 IC h=3m being strong: monthly rebal ALREADY captures 3m-forward-expected-return rolling because each entry's holding period is the time-to-next-rebal. Smoothing the FACTOR doesn't help; it just adds noise from stale data. **Stage 1b monthly stays the best honest variant.** |
| **12** | **TOP_N=10 concentration test** — iter-7 tested 30 (worse), this tests 10 (more concentrated). With Stage 1 foundation. | `--top-n 10` flag added to `run_universe_backtest.py`. | OOS **9.33%** (vs Stage 1b TOP_N=15 10.50%, -1.17pp) / IS 3.76% / MaxDD -46.6% / Sharpe 0.48 / final 3.02 / 2008 -32.4% / 2015 +124% / 2020 +13.9% / 2022 +11.8% / 2024 +4.4%. | **NEUTRAL / NO IMPROVEMENT** | Concentration tested both ways now: TOP_N=30 (iter-7 OOS 10.09%) and TOP_N=10 (iter-12 OOS 9.33%) both UNDERPERFORM TOP_N=15 (Stage 1b OOS 10.50%). 15 is the local sweet spot. But the differences (±1pp) are within bootstrap CI overlap and not statistically meaningful. **Concentration is not the source of alpha; signal weakness is.** Final iter-12 also confirms the iter-7 finding from the post-Stage-1 foundation. |
