...
> **Date:** 2026-06-14; updated 2026-06-17 to reflect the data-gate/reconciliation work already shipped.
...
> `docs/research/serenity/SCAN_1_AI_SEMIS.md` (the discovery layer) · `scripts/deep_thesis_reconcile.py`
> (PR-G v0 primary reconciliation gate) · `scripts/universe_data_health.py` + the `DATA_BLOCKED` compose/register
> enforcement (fresh committed price gate) · the standing no-validated-alpha discipline
...
**Freshness enforcement is now live (PR #89-#92).** If the composer uses `universe_a_snapshot_fallback` and
`scripts/universe_data_health.py` returns `BLOCKED` (stale or otherwise untrustworthy snapshot), `decision_sheet.py`
stamps `DATA_BLOCKED` and `sheet_checkpoints.py --register` refuses the sheet even if the red-team record would
otherwise match. A real committed OHLC price bypasses the universe fallback gate; this prevents false-blocking good
price evidence while still refusing stale fallback prices.
...
- **v0.1 (still pending):** `register()` should AUTO-STAMP `quality.human_red_team = PASS(...)` and re-render the composer MD as part of registration, so no future sheet can go stale and the earlier sheets (BYD/华海/鼎龙) get corrected on their next touch. This is **not** the same as PR-G v0 (#86), which shipped the primary reconciliation gate.
...
## 5. The pipeline (PR-G done in v0 / PR-H / PR-I) — and why extraction fidelity is the whole game
...
- **一手对账门 (primary-reconciliation gate):** **v0 is DONE in #86** (`scripts/deep_thesis_reconcile.py`).
  It reconciles load-bearing thesis inputs against primary/committed evidence and can grade a sheet as
  `RED_TEAM_GRADE` or `NEEDS_CORRECTION_BEFORE_REDTEAM`. Future extractor output must pass the same gate before
  it is eligible for human red-team.
...
### 5.2 PR-G status — reconciliation shipped; extractor still pending
**Shipped v0 (#86):** `scripts/deep_thesis_reconcile.py` — the primary reconciliation gate. It already caught and
corrected a real over-assertion in 胜宏 (AI mix was E2, not disclosed) and correctly released a false-flag on 工业富联
(Q1 105.95亿 verified-clean). This is now part of the factory's fact ground.

**Pending v1:** `scripts/deep_thesis_prepare.py` (the extractor).
...
all 6 registered names WATCH/watch-constructive because price ran to the bull case or R/R stayed below 2:1), so to find `STARTER_CANDIDATE` names (good thesis + **good price**)
...
1. **Sample size** — 6 ACTIVE, far from the 15–30 the process gate needs.
...
2. **PR-G v0 (#86, done)** — `deep_thesis_reconcile.py` primary reconciliation gate.
3. **PR-G v1 (pending)** — `deep_thesis_prepare.py` extractor + golden-regression test (reproduce 澜起/拓荆) + the §3.3 v0.1 `register()` auto-stamp.
4. **PR-H** — earnings-bridge JSON schema + composer export.
5. **PR-I** — batch upgrade queue, orthogonal-theme first; emit per-name stance from the §4.1 state machine.
6. **(later)** shadow model portfolio (§4.5) once a `STARTER_CANDIDATE` first appears.
...
4. **Extractor scope (PR-G v1)** — committed-data + filing-PDF only, or also a live-quote fetch (with the §3.1 labeling) so the reconciliation gate has a second source?
