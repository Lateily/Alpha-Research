...
| 状态 | **RATIFIED(Junyan 2026-06-12,#72 复审)** — 家族即时锁定:规则/阈值不可再调,失败即 KILL/NO-CLAIM,修改 = 新家族。**边界(Junyan):SUE≥1.0 / 50d / K=20 / −15% 等阈值仍是 [unvalidated intuition],只有正式 backtest + 19-gate 之后才有资格谈是否保留** |
...
## 9. Open decisions — **全部已裁决(Junyan 2026-06-12):① 每日数据 = A 本地 runner ② NO-CLAIM-但胜随机对照 ⇒ 只进 Factory A 事件雷达,永不进交易推荐 ③ paper 观察期 = 30 个交易日**
