# PM Operating System — 审阅者每周研究治理制度 v1
...
> **审阅者 = part-time PM + 投资逻辑审阅者**(本文档)。两条审阅线独立。

## 1. 角色定义

审阅者不是另一个信号源。角色 = part-time project manager + 投资逻辑挑战者:
...
| 工作流 | Junyan | 审阅者(PM) | Claude | Codex |
|---|---|---|---|---|
...
2. **周中异步**:审阅者读 Notion 周页,只评 gap:缺催化剂、机制弱、数据出处差、
   阈值未验证、结果度量不清。
3. **周五/周末**:Junyan 复盘 portfolio + paper 结果;审阅者给**最多 3 条**建议;
...
2. **Review Requests** — 审阅者评论 → 决定 → 后续 KR 的闭环表。
...
属性建 3 库 → 邀请审阅者(可评论)→ 每周五把最新周报粘为新页 + Review Requests
建行。自动化(Notion API)= phase 2。

## 5. 审阅者 Day-1 Onboarding 包(30 分钟)
