...
> **Shipped (no longer queued):** v1 fswatch trio activated 2026-05-02 in
> commit `e347129`. Watcher scripts live in `bin/agent-watch-{codex,reviewer,design}.sh`.
> The preserved Codex draft remains at `v1-implementation-codex-draft/` for
> historical reference.

1. **Vercel CLI integration** (Junyan flagged 2026-05-02)
...
2. **Real-time price refresh** (Phase 3 of Universe Browser)
...
3. **K-line lazy financial data** (Phase 2 extension)
...
4. **HKEx endpoint reverse-eng** — see fetch_hkex.py module docstring
...
5. **Step 8 phase_timing UI surface in Dashboard**
...
6. **Real-time data for entire universe** (the 同花顺 replacement scope)
