...
function Research({ L, lk, ticker, stocks: stocksMap, open, toggle, C, liveData, eqrData, rdcfData, factcheckData, pulse, pulseLoading, onRunPulse, onGenerateResearch, signalsData, scissorsData, liData, egapScores }) {
  ...
  useEffect(() => {
    ...
    const restrictedSharesFetches = watchlistTargets
      .then(targets => Promise.all(targets.map(t =>
          fetchJson(`data/restricted_shares/${t}.json`).then(data => ({ ticker: t, data }))
        )));
    const repurchaseFetches = watchlistTargets
      .then(targets => Promise.all(targets.map(t =>
    ...
    Promise.all([
      ...
      const restrictedSharesDataMap = { [ticker]: null };
      (restrictedSharesArr || []).forEach(({ ticker: tk, data }) => { if (data) restrictedSharesDataMap[tk] = data; });
      setRestrictedSharesData(prev => ({ ...prev, ...restrictedSharesDataMap }));
  ...
  const RestrictedSharesCard = ({ data, ticker }) => {
    if (!data || data._status === 'skipped_hk' || data._status === 'skipped_us') return null;
    if (data._status === 'empty') {
      return (
        <div style={{padding:'4px 8px', background:C.soft, border:`1px solid ${C.border}`, borderRadius:6, marginBottom:10, fontSize:10, color:C.mid}}>
    ...
    const events = Array.isArray(data.events)
      ? [...data.events]
          .sort((a, b) => dateKey(a?.float_date).localeCompare(dateKey(b?.float_date)))
          .slice(0, 4)
      : [];
    const totalRatio = finiteNumber(summary.total_float_ratio_180d);
    const upcomingCount = finiteNumber(summary.upcoming_count);
    const riskLevel = summary.risk_level || null;
    const riskColor = riskLevel === 'HIGH' ? C.red
      : riskLevel === 'MED' ? C.gold
      : riskLevel === 'LOW' ? C.green
    ...
    const riskLabel = riskLevel === 'HIGH' ? 'HIGH'
      ...
      : riskLevel === 'LOW' ? 'LOW'
      : 'NO DATA';
    const formatFloatDate = (value) => {
      const s = String(value || '');
      if (/^\d{8}$/.test(s)) return `${s.slice(0,4)}-${s.slice(4,6)}-${s.slice(6,8)}`;
      if (/^\d{4}-\d{2}-\d{2}$/.test(s)) return s;
      return s || '—';
    };
    const formatFloatRatio = (value) => {
      const n = finiteNumber(value);
      return n == null ? '—' : `${n.toFixed(1)}%`;
    };
    const ratioColor = (value) => {
    ...
    const hasShareType = events.some(row => row?.share_type);
    const hasHolder = events.some(row => row?.holder_name);
    const tableGrid = ['82px', '58px', hasShareType ? 'minmax(54px, 1fr)' : null, hasHolder ? 'minmax(72px, 1fr)' : null]
      .filter(Boolean)
      .join(' ');
    const nextUnlockDate = formatFloatDate(summary.next_unlock_date);

    return (
      <div style={{padding:'10px 12px', background:C.card, border:`1px solid ${C.border}`, borderRadius:12, boxShadow:SHADOW, marginBottom:10}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:10, marginBottom:8}}>
          <div style={{fontWeight:700, fontSize:11, color:C.dark}}>限售解禁</div>
          <div title={ticker} style={{fontSize:9, color:C.mid, fontFamily:MONO, textAlign:'right', background:C.soft, border:`1px solid ${C.border}`, borderRadius:4, padding:'1px 5px'}}>
            {nextUnlockDate}
          </div>
        </div>
        <div style={{width:'100%', borderRadius:8, background:`${riskColor}10`, border:`1px solid ${riskColor}30`, padding:10, marginBottom:10}}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', gap:10}}>
            <div>
              <div style={{fontFamily:MONO, fontSize:22, lineHeight:1, color:riskColor, fontWeight:800}}>
                {formatFloatRatio(totalRatio)}
              </div>
              <div style={{fontSize:9, color:C.mid, marginTop:4}}>未来180天解禁占比</div>
            </div>
            <div style={{display:'flex', flexDirection:'column', alignItems:'flex-end', gap:6}}>
              <span style={{background:`${riskColor}30`, color:riskColor, borderRadius:4, padding:'2px 6px', fontSize:10, fontWeight:700, fontFamily:MONO}}>
                {riskLabel}
              ...
              <span style={{fontSize:10, color:C.mid, fontFamily:MONO}}>
                {upcomingCount != null ? Math.round(upcomingCount) : '—'} 起
              </span>
            </div>
          </div>
        </div>
        {events.length > 0 && (
          <div>
            <div style={{display:'grid', gridTemplateColumns:tableGrid, gap:6, padding:'0 0 4px', borderBottom:`1px solid ${C.border}`, fontSize:9, color:C.mid, fontWeight:700}}>
              ...
              {hasShareType && <span>类型</span>}
              {hasHolder && <span>股东</span>}
            </div>
            {events.map((event, i) => (
              <div key={`${event.float_date || 'date'}-${i}`} style={{display:'grid', gridTemplateColumns:tableGrid, gap:6, padding:'4px 0', borderBottom:i < events.length - 1 ? `1px dashed ${C.border}` : 'none', fontSize:10, alignItems:'center'}}>
                <span style={{fontFamily:MONO, fontSize:10, color:C.dark}}>{formatFloatDate(event.float_date)}</span>
                <span style={{fontFamily:MONO, fontSize:10, color:ratioColor(event.float_ratio), textAlign:'right', fontWeight:700}}>
                  {formatFloatRatio(event.float_ratio)}
                </span>
                {hasShareType && (
                  <span style={{fontSize:10, color:C.mid, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>
                    {event.share_type || '—'}
  ...
  const BuybackCard = ({ data, ticker }) => {
    ...
    const signalColor = signalLevel === 'HIGH' ? C.green
      : signalLevel === 'MED' ? C.gold
      : C.mid;
    const signalLabel = signalLevel === 'HIGH' ? 'HIGH'
      : signalLevel === 'MED' ? 'MED'
      : signalLevel === 'LOW' ? 'LOW'
      : 'NO DATA';
    const totalCount = finiteNumber(summary.total_count);
    const totalAmount = finiteNumber(summary.total_amount_180d);
    const formatBuybackDate = (value) => {
      const s = String(value || '');
      if (/^\d{8}$/.test(s)) return `${s.slice(0,4)}-${s.slice(4,6)}-${s.slice(6,8)}`;
      if (/^\d{4}-\d{2}-\d{2}$/.test(s)) return s;
      return s || '—';
    };
    const formatBuybackAmount = (value) => {
      const n = finiteNumber(value);
      if (n == null) return '—';
      const sign = n < 0 ? '−' : '';
      const abs = Math.abs(n);
    ...
    const truncateProc = (value) => {
      const s = String(value || '').trim();
      if (!s) return '—';
      return s.length > 6 ? `${s.slice(0, 6)}…` : s;
    };
    const hasProc = events.some(row => String(row?.proc || '').trim());
    const tableGrid = ['82px', '74px', hasProc ? 'minmax(58px, 1fr)' : null]
      .filter(Boolean)
      .join(' ');
    const lastAnnounceDate = formatBuybackDate(summary.last_announce_date);

    return (
      <div style={{padding:'10px 12px', background:C.card, border:`1px solid ${C.border}`, borderRadius:12, boxShadow:SHADOW, marginBottom:10}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:10, marginBottom:8}}>
          <div style={{fontWeight:700, fontSize:11, color:C.dark}}>回购</div>
          <div title={ticker} style={{fontSize:9, color:C.mid, fontFamily:MONO, textAlign:'right', background:C.soft, border:`1px solid ${C.border}`, borderRadius:4, padding:'1px 5px'}}>
            {lastAnnounceDate}
          </div>
        </div>
        <div style={{width:'100%', borderRadius:8, background:`${signalColor}10`, border:`1px solid ${signalColor}30`, padding:10, marginBottom:10}}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', gap:10}}>
            <div>
              <div style={{fontFamily:MONO, fontSize:22, lineHeight:1, color:signalColor, fontWeight:800}}>
                {formatBuybackAmount(totalAmount)}
              </div>
              <div style={{fontSize:9, color:C.mid, marginTop:4}}>180天累计回购</div>
            </div>
            <div style={{display:'flex', flexDirection:'column', alignItems:'flex-end', gap:6}}>
              ...
              <span style={{fontSize:10, color:C.mid, fontFamily:MONO}}>
                {totalCount != null ? Math.round(totalCount) : '—'} 起
              </span>
              {summary.active_program === true && (
                <span style={{fontSize:9, fontFamily:MONO, background:`${C.green}20`, color:C.green, padding:'1px 4px', borderRadius:4}}>
                  进行中
                </span>
              )}
            </div>
        ...
        {events.length > 0 && (
          <div>
            <div style={{display:'grid', gridTemplateColumns:tableGrid, gap:6, padding:'0 0 4px', borderBottom:`1px solid ${C.border}`, fontSize:9, color:C.mid, fontWeight:700}}>
              ...
              {hasProc && <span>状态</span>}
            </div>
            {events.map((event, i) => (
              <div key={`${event.ann_date || 'date'}-${i}`} style={{display:'grid', gridTemplateColumns:tableGrid, gap:6, padding:'4px 0', borderBottom:i < events.length - 1 ? `1px dashed ${C.border}` : 'none', fontSize:10, alignItems:'center'}}>
                <span style={{fontFamily:MONO, fontSize:10, color:C.dark}}>{formatBuybackDate(event.ann_date)}</span>
                <span style={{fontFamily:MONO, fontSize:10, color:C.dark, textAlign:'right', fontWeight:700}}>
                  {formatBuybackAmount(event.amount)}
                </span>
                {hasProc && (
                  <span title={event.proc || ''} style={{fontSize:10, color:C.mid, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>
                    {truncateProc(event.proc)}
                  </span>
                )}
              </div>
            ))}
...
// ─── Factory Progress (2026-06-12, Junyan ask: 实时追踪工程进度) ───
// Read-only view over public/data/factory_progress.json (generated by
// scripts/factory_progress.py from committed artifacts). UNVALIDATED pipeline
// state — never a recommendation surface.
function FactoryProgress({ L, lk, C }) {
  const [fp, setFp] = useState(null);
  const [err, setErr] = useState(null);
  useEffect(() => {
    fetch(`${import.meta.env.BASE_URL}data/factory_progress.json`)
      .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); })
      .then(setFp).catch(e => setErr(String(e)));
  }, []);
  ...
  return (
    <div style={{ maxWidth: 980, margin: '0 auto' }}>
      ...
      <div style={card}>
        <div style={h}>{L('Factory A — Research (Core Thesis forward validation)', '研究工厂 — 决策书前向验证')}
          {pill(`${fa.n_active ?? 0} ACTIVE`, C.green)}
          {fa.next_read && pill(L(`next read ${fa.next_read.date} (${fa.next_read.days_to_due}d)`,
                                  `下一读数 ${fa.next_read.date}(${fa.next_read.days_to_due} 天后)`), C.blue)}
        </div>
        {(fa.registrations || []).map(r => (
          <div key={r.ticker} style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, padding: '8px 0', borderTop: `1px solid ${C.border}` }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {(r.checkpoints || []).map(c => (
                <span key={c.offset_days} style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4,
                  border: `1px solid ${c.status === 'EVALUATED' ? C.green : C.border}`,
                  color: c.status === 'EVALUATED' ? C.green : C.mid }}>
                  +{c.offset_days}d {c.due_date}{c.status === 'PENDING' && c.days_to_due != null ? ` (${c.days_to_due}d)` : ' ✓'}
                </span>
              ))}
            </div>
        ...
        <div style={{ ...small, marginTop: 6 }}>{fa.note_zh}</div>
      </div>

      <div style={card}>
        <div style={h}>{L('Discovery — Serenity scan', '发现层 — Serenity 扫描')} {pill(dis.scan_id || '—', C.blue)}</div>
        <div style={small}>{dis.theme} · {dis.scan_date} · {L(`${dis.pool_n} names / ${dis.layers_n} layers`, `${dis.pool_n} 名候选池 / ${dis.layers_n} 层链排序`)} · {dis.status}</div>
        ...
        <div style={{ ...small, marginTop: 6 }}>★ = {L('nominated into decision sheets', '已提名进决策书')} · {dis.label}</div>
      </div>

      <div style={card}>
        <div style={h}>{L('Factory B — Quant strategy families', '交易工厂 — 量化策略家族')}</div>
      ...
      <div style={card}>
        <div style={h}>{L('Milestones', '里程碑(最近优先)')}</div>
...
function DailyModelPortfolio({ L, lk, C, onSelect }) {
  ...
  // (generated by scripts/model_portfolio.py: fresh pilot, no legacy paper positions,
  // thesis-derived/uncalibrated targets). Binding the UI to the run/candidate schema
  // means user decisions (Follow/Modify/Reject/Watch + reason + actual trade; PR #42)
  ...
  const ModelCard = ({ r }) => {
    ...
    const inv = (r.risk_rules && Array.isArray(r.risk_rules.invalidation) && r.risk_rules.invalidation.length)
      ? String(r.risk_rules.invalidation[0]).split('|')[0].trim() : null;
    return (
      <div style={{ ...cardBox, borderLeft: `4px solid ${dc}` }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 700, color: dc, background: `${dc}1A`, border: `1px solid ${dc}40`, borderRadius: 4, padding: '1px 7px' }}>{r.model_action}</span>
          ...
          {r.validation_status ? <span style={{ fontSize: 9, color: warn }}>{r.validation_status}</span> : null}
        </div>
        {r.why ? <div style={{ fontSize: 10.5, color: C.mid, lineHeight: 1.5, marginBottom: 6 }}><b style={{ color: C.dark }}>{L('Why', '推荐理由')}: </b>{String(r.why).slice(0, 170)}</div> : null}
        {r.construction_plan ? <div style={{ fontSize: 10.5, color: C.mid, lineHeight: 1.5, marginBottom: 6 }}><b style={{ color: C.dark }}>{L('Construction', '构仓')}: </b>{String(r.construction_plan).slice(0, 150)}</div> : null}
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr 1fr', gap: 8, marginBottom: 8 }}>
          <div style={{ fontSize: 10, color: C.mid }}>
            ...
            <span>{L('Thesis-derived, not calibrated', '论点推导、未校准')}{tr.reward_to_risk != null ? ` · R/R ${tr.reward_to_risk}` : ''} · {L('entry ref: pending next sync', '入场参考：待下次同步')}</span>
          </div>
          <div style={{ fontSize: 10, color: C.mid }}>
            <div style={{ fontWeight: 700, color: C.dark, marginBottom: 1 }}>{L('Quant signal', '量化信号')}</div>
            {q ? <span>{q.action} · {q.rationale || ''}</span> : <span>—</span>}
          </div>
          <div style={{ fontSize: 10, color: C.mid }}>
            <div style={{ fontWeight: 700, color: C.dark, marginBottom: 1 }}>{L('Invalidation', '失效条件')}</div>
            {inv ? <span>{inv.slice(0, 95)}</span> : <span>—</span>}
          </div>
        </div>
  ...
  return (
    <div style={{ padding: '14px 18px', maxWidth: 960, margin: '0 auto', overflowY: 'auto' }}>
      ...
      <div style={{ fontSize: 9.5, color: C.mid, margin: '-4px 0 14px', lineHeight: 1.5 }}>
        {L(`Fresh pilot book${asOf ? ` · run ${asOf}` : ''}${pilotStart ? ` · live tracking from ${pilotStart}` : ''} — no legacy paper positions inherited.`,
           `全新 pilot 账本${asOf ? ` · 运行日 ${asOf}` : ''}${pilotStart ? ` · 实操收益自 ${pilotStart} 起算` : ''} —— 不继承任何旧模拟仓位。`)}
      </div>
      ...
      {(() => {
        const myDec = Object.values(decisions).filter(x => x && x.run_date === asOf && (x.status || x.reason || x.actual_trade));
        const exportObj = { run_date: asOf, pilot_start_date: pilotStart, decisions: myDec };
        const copyJson = () => { try { navigator.clipboard.writeText(JSON.stringify(exportObj, null, 2)); } catch {} };
        const downloadJson = () => {
          const blob = new Blob([JSON.stringify(exportObj, null, 2)], { type: 'application/json' });
          const url = URL.createObjectURL(blob);
          const a = document.createElement('a'); a.href = url; a.download = `model_decisions_${asOf || 'run'}.json`; a.click();
          URL.revokeObjectURL(url);
        };
        return (
          <div style={{ ...cardBox, marginTop: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: myDec.length ? 8 : 0 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: C.dark }}>{L('Your execution log', '你的执行记录')} <span style={{ color: C.mid, fontWeight: 500 }}>({myDec.length})</span></div>
              <span style={{ fontSize: 9, color: C.mid }}>{L(`run ${asOf} · saved in this browser; export and send to Junyan to feed the validation loop`, `运行日 ${asOf} · 仅存本浏览器；导出发给 Junyan 喂验证闭环`)}</span>
              {myDec.length > 0 ? (
                <>
                  <button onClick={copyJson} style={{ fontSize: 10, fontWeight: 600, padding: '4px 12px', borderRadius: 6, border: `1px solid ${C.border}`, background: 'transparent', color: C.dark, cursor: 'pointer' }}>{L('Copy JSON', '复制 JSON')}</button>
                  <button onClick={downloadJson} style={{ fontSize: 10, fontWeight: 600, padding: '4px 12px', borderRadius: 6, border: `1px solid ${C.border}`, background: 'transparent', color: C.dark, cursor: 'pointer' }}>{L('Download JSON', '下载 JSON')}</button>
                </>
              ) : null}
            </div>
            {myDec.map(x => (
              <div key={`${x.sleeve || 'core_thesis'}|${x.ticker}`} style={{ borderTop: `1px solid ${C.border}`, padding: '6px 0', fontSize: 10, color: C.mid }}>
                ...
                {x.actual_trade ? <span style={{ marginLeft: 6, color: C.dark }}>· {L('trade', '交易')}: {x.actual_trade}</span> : null}
              </div>
            ))}
          </div>
        );
...
export default function Dashboard() {
  ...
  const TABS = [
    ...
    { id:'factory',  label:L('Factory','工厂进度'), icon:<Database size={14}/> },
    ...
    <div style={{display:'flex', height:'100vh',
      ...
      <div style={{flex:1, minWidth:0, display:'flex', flexDirection:'column', overflow:'hidden'}}>
        ...
        <div style={{flex:1, minWidth:0, overflowY:'auto', padding:`14px 20px ${dark?'36px':'14px'} 20px`, background:C.bg}}>
          ...
          {tab==='factory'  && <FactoryProgress L={L} lk={lk} C={C}/>}
        </div>
      </div>
