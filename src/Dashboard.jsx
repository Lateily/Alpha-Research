  ...
  const runResearch = async () => {
    ...
    try {
      ...
      setLastResearchMeta({
        ...
        // The envelope _status is the gate's verdict over EVERY returned block; the
        // synthesis receipt alone can read PASS while a sub-thesis is held. The capped
        // _quality carries the suspect total across all blocks for the same reason.
        fact_check_status:  json._status || json.data?._fact_check?.status || null,
        fabrication_suspects: json.data?._quality?.fabrication_suspects
                              ?? json.data?._fact_check?.summary?.fabrication_suspects ?? null,
