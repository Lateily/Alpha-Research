...
- **#4 status**: ✅ ACCEPTED (Junyan PASS + name polish applied: `load_registered()` now carries `name` → 大参林/Geely resolve, no null-name rows). read-only shadow portfolio comparison.
- **v0 COMMIT**: `d2ec27f core(factory): CORE Alpha Factory v0 — screen, pre-register, shadow` on `feature/core-alpha-factory-v0`. 15 files, 5950 insertions. Protected positions/analytics/snapshots.json NOT staged (still dirty from daily sim). **Not pushed** (Junyan said commit). Staged set verified = exactly the factory bundle.
- **Cull/promote criteria**: ✅ v0 RATIFIED (Junyan 2026-05-30, 4 revisions applied) `docs/strategy/CORE_CULL_PROMOTE_CRITERIA_v0.md`. Revisions: (1) sample = ≥8 **independent directional families** at 60d, NOT 3 horizons of one thesis; (2) ±8% = **raw 60d** theme-peer residual, not annualized; (3) BY p<0.10 = `PROMOTE_CANDIDATE` only + p<0.05 strong-signal split, capital still human; (4) **layered wrong_if** (single ticker-family firing retires only that family; bucket needs ≥2 same-mechanism). + harder promote≠capital. Numbers stay `[unvalidated intuition]`. forward_evidence_tier = v0.1 after PR (before first formal verdict).
- **#4 status (history)**: BUILT → PASS + name polish → ACCEPTED → committed in d2ec27f.
